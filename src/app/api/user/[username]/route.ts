...
export const GET = async (req: NextRequest, { params }: { params: { username: string } }) => {
    ...
    if (req.method === 'GET') {
        try {
            ...
            const totalPosts = await Post.countDocuments({ "userInfo.authorId": user._id });
            const userPosts = await Post.find({ "userInfo.authorId": user._id }).skip(skip).limit(limit);
...
export const PUT = async (req: any, { params }: { params: { username: string } }) => {
    await connectDb();
    try {
        const username = params.username;
